...
```
┌─────────────────┐
│    contxt      │  TypeScript CLI (MCP Client)
└────────┬────────┘
...
| Config | TOML (`~/.contxt/config.toml`) | Human-readable |
| Memory | Plain `.md` files in `~/.contxt/reports/` | No database |
...
```bash
contxt init                  # Generate config file interactively
contxt run                   # Generate report to stdout
contxt run > report.md       # Pipe to file
contxt run --dry             # Show discovered tools, skip LLM call
contxt history               # List past saved reports
contxt schedule --every "9am"    # Write crontab/launchd entry
contxt schedule --every "*/15m"  # Every 15 minutes
contxt schedule --every "*/6h"   # Every 6 hours
```
...
- After each run, optionally save the report to `~/.contxt/reports/YYYY-MM-DD.md`
...
- Save reports to `~/.contxt/reports/`
- Load past reports as context
- `contxt history` command
- Decision Trail section

### Phase 4: Scheduling + Polish

- `contxt schedule` writes crontab/launchd entry
- `contxt run --dry` for debugging
...
| Phase | Test |
|-------|------|
| 1 | `ANTHROPIC_API_KEY=xxx GITHUB_TOKEN=xxx contxt run` outputs GitHub activity report |
| 2 | Jira ticket IDs in GitHub PRs appear linked in report |
| 3 | Second-day report references decisions from first-day report |
| 4 | `contxt schedule --every "9am"` creates valid crontab entry |
